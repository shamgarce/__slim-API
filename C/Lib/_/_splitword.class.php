...
$res = $sp->GetIndexText($sp->SplitRMM($str),20);
if(!$sp->is_utf8($res)) $res = iconv( 'gbk','utf-8//IGNORE', $res);
echo $res."<br>";
$sp->Clear();
$t3 = ExecTime();
$t0 = $t3-$t2;
...
class SplitWord
{
	var $TagDic 	= Array();
	var $RankDic 	= Array();
	var $Dic_ts 	= Array();		//特殊词字典 add by sham
	var $Dic_ts_ci 	= Array();		//特殊词 add by sham
	//=======================================================================
  ...
  function __construct(){  	  	
  	...
  	//高级分词，预先载入词典以提分词高速度
  	$dicfile 	= dirname(__FILE__)."/dic/dededic.csv"; 
	$dicfile_ts = dirname(__FILE__)."/dic/pinpai.csv"; 		//特殊词
	...
	$i = 0;
  	while($line = fgets($fp,256)){
		$i++;
		$ws = explode(' ',$line);
		//$this->TagDic[$ws[0]] = $ws[1];						//没有起作用,注释 by sham
	...
	//------------------------------
	//特殊词字典
	$fp = fopen($dicfile_ts,'r');
	while($line = fgets($fp,256)){
		$ws = explode(' ',$line);
		$this->Dic_ts[$ws[0]] = $ws[2];
	}
	fclose($fp);
  }
  ...
  //RMM分词算法
  //-----------------------------
  function SplitRMM($str=""){				//如果输入utf8 转化为gbk 最后输出gbk
	if($this->is_utf8($str)) $str = iconv('utf-8//IGNORE', 'gbk', $str);
	...
	//print_r($this->Dic_ts);
	//循环查找特殊词,找到,进行记录,并且
	foreach($this->Dic_ts as $ks =>$vs){
		if(strpos($str,$ks)!==false){
			//echo $ks.'1';
			//找到特殊词,记录,并且词源替换
			$ci_ts[] = $ks;
	...
	$this->Dic_ts_ci = $ci_ts;
	
//print_r($this->Dic_ts_ci);	
	//======================================================
	//对文本进行粗分
  ...
  {
	if($this->is_utf8($okstr)) $str = iconv('utf-8//IGNORE', 'gbk', $okstr);
    if($okstr=="") return "";
    $ws = explode(" ",$okstr);
    $okstr = "";
    $wks = "";
	...
	if(!empty($this->Dic_ts_ci)){
		foreach($this->Dic_ts_ci as $vs){
			$okstr .= $vs." ";
		}
	}
    if(is_array($wks))

...
class Manm extends CI_Controller
{
...
//		$this->db = Db::getInstance();
	...
	{
		//对该ID进行编辑
		$data['row'] = $this->S->db->getrow("select * from userapi where id = $id");
		//==========================================================
...
//		$rc['response'] = str_replace("'","\"",$rc['response']);
		...
		$this->S->db->autoExecute('userapi',$rc,'UPDATE',"ID = {$_POST['id']}");
		$res['code'] = 200;
		$res['msg'] ='完成';
	...
	{
		$rc['debug'] = $rel;
		$this->S->db->autoExecute('userapi',$rc,'UPDATE',"ID = $id");
		$res['code'] = 200;
		$res['msg'] ='完成';
		echo json_encode($res);
		//==========================================================
	...
	public function addnew_exc()
	{
		...
		$this->S->db->autoExecute('userapi',$rc,'INSERT');
		$res['code'] = 200;
		$res['msg'] ='完成';
		echo json_encode($res);
	...
	{
		$rc = $this->S->db->getall("select m from user_model order by m");
		//==========================================================
		$data['rc'] = $rc;
	...
	public function Model_add_exe()
	{
		...
		$this->S->db->autoExecute('user_model',$rc,'INSERT');
		$res['code'] = 200;
		$res['msg'] ='完成';
		echo json_encode($res);
	}
	...
	{
		$id = $_POST['id'];
		$this->S->db->query("delete from user_model where id = $id");

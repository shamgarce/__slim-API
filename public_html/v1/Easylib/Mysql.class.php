...
class Mysql{
    ...
    public $root_path           = 'EasyCache/mysql/';//RHCACHE
    ...
    private function __construct($config){
        $this->root_path = MBASE.$this->root_path;
    ...
    public static function getInstance($config=array()){
        !(self::$instance instanceof self)&&self::$instance = new Mysql($config);
...
//    }
    ...
    private function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        ...
        if ($pconnect){
            if (!($this->link_id = @mysql_pconnect($dbhost, $dbuser, $dbpw))){      //持久连接
        ...
        /* 如果mysql 版本是 4.1+ 以上，需要对字符集进行初始化 / 在这里,不针对5.3一下进行判断*/
    ...
    //==================================================
    //内容截取
    private function content_cut($str,$startstr="",$endstr=""){
        $outstr="";
    ...
    //==================================================
    //获取缓存的数据
    private function getsqldata($sql,$cacheid){
    ...
    //==================================================
    //设置缓存的数据
    private function setsqldata($sql,$cacheid,$data){
    ...
    //==================================================
    //获取缓存的数据
    private function sqliscached($sql,$cacheid){
        if($this->lifetime <= 0){return false;}
        if($this->cache_data_name == $sql && $this->cacheid	== $cacheid){return true;}		//获取
        $cachefile = $this->getcachefilename($sql,$cacheid);
...
//===================================================================
    ...
    public function getAll($sql,$str=''){
        if($this->sqliscached($sql,'getall'.$str)) return $this->getsqldata($sql,'getall'.$str);
        $res = $this->query($sql);
        if ($res !== false){
            $arr = array();
